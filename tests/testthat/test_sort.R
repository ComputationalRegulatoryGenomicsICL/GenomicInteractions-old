...
test_that("Sort returns correctly reversed anchors", {
  gi <- new("GenomicInteractions",
            ...
            anchor_one = GRanges(seqnames = Rle(factor(c("chr1", "chr2", "chr1")), c(1, 2, 2)),
                                 ranges = IRanges(1:5, width = 10:6),
                                 strand = Rle(strand(c("+", "+", "-", "-", "-"))),
                                 seqinfo = Seqinfo(seqnames = paste("chr", 1:2, sep=""))),
            anchor_two = GRanges(seqnames = Rle(factor(c("chr1", "chr2", "chr1")), c(2, 2, 1)),
  ...
  expect <- new("GenomicInteractions",
                metadata = list(experiment_name="test", description = "this is a test"),
                anchor_one = GRanges(seqnames = Rle(factor(c("chr1", "chr2", "chr1")), c(2, 1, 2)),
                                     ranges = IRanges(start = c(1,6,3,4,3), end = c(10,14,10,10,8)),
                                     strand = Rle(strand(c("+", "+", "-", "-", "-"))),
                                     seqinfo = Seqinfo(seqnames = paste("chr", 1:2, sep=""))),
                anchor_two = GRanges(seqnames = Rle(factor(c("chr1", "chr2", "chr1")), c(1, 3, 1)),
...
test_that("Sort returns correctly reversed and ordered anchors", {
  ...
  expect <- new("GenomicInteractions",
                metadata = list(experiment_name="test", description = "this is a test"),
                anchor_one = GRanges(seqnames = Rle(factor(c("chr1", "chr2")), c(4, 1)),
                                     ranges = IRanges(start = c(1,6,3,4,3), end = c(10,14,8,10,10)),
                                     strand = Rle(strand(c("+", "+", "-", "-", "-"))),
                                     seqinfo = Seqinfo(seqnames = paste("chr", 1:2, sep=""))),
                anchor_two = GRanges(seqnames = Rle(factor(c("chr1", "chr2", "chr1", "chr2")), c(1,1,1,2)),
